...
# Root-level ``for_solver/`` artifacts left by the OLD flat layout
# (:func:`write_for_solver`) that re-expose every capsule/task and thus
# violate the per-capsule binding contract. The per-capsule writer purges
# exactly these known names (files and the ``capsules`` symlink); it never
# touches the ``capsule-*`` dirs or ``index.jsonl``.
...
def _purge_legacy_root_artifacts(for_solver_dir: Path) -> list[str]:
    """Remove old flat-layout root artifacts (:data:`_LEGACY_ROOT_ARTIFACTS`).

    The pre-per-capsule :func:`write_for_solver` shape left a global
    ``tasks.jsonl`` / ``submission.schema.json`` / ``submission.example.json``
    and a ``capsules`` symlink at the ``for_solver/`` root — each re-exposing
    ALL capsules/tasks, breaking the one-capsule binding contract. This
    removes exactly those known names (regular files and the symlink),
    leaving the per-capsule ``capsule-*`` dirs and ``index.jsonl`` intact.
    Returns the removed paths as strings.
    """
    removed: list[str] = []
    for name in _LEGACY_ROOT_ARTIFACTS:
        target = for_solver_dir / name
...
def write_for_solver_per_capsule(
    *,
    ...
    Before anything is written, leftover root artifacts from the OLD flat
    layout (:data:`_LEGACY_ROOT_ARTIFACTS` — a global ``tasks.jsonl``,
    ``submission.*``, and a ``capsules`` symlink that re-expose every
    capsule) are purged, so the tree obeys the per-capsule contract (only
    ``capsule-*/`` dirs + ``index.jsonl``).
